...
export class ManageWalletComponent implements OnInit {
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `https://my.nos.cash/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarning(`Wallet must be unlocked`);
    const fileName = `NOSwallet-AddressBook.json`;
  ...
  exportToFile() {
    ...
    const fileName = `NOSwallet-Wallet.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendError(`Bad import data, make sure you selected a NOSwallet Address Book export`)
        }
